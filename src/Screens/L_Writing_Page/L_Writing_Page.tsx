...
const LWritingPage: FunctionComponent = observer(() => {
    ...
    // const GPT_PROMPT = new_writing_prompt({
    //     fullname: UserInfoStore.user_info?.fullname as string,
    // });
    const GPT_PROMPT: string = AdminStore.admin_data?.writing_prompt
        ?.replace(
            '/--FULLNAME--/',
    ...
    useEffect(() => {
        ...
        if (timer === 8 && !hasSkipped) {
            const convey_mssg =
                "Thank you for working hard on the Writing Session, now let's move on to the last session.";
            setHideInput(true);
            setAllMessages(prev => [
                ...prev,
                {
    ...
    }, [timer, hasSkipped, route.params, navigation]);
