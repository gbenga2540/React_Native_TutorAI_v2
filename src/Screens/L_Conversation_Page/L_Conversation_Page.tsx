...
import { get_random_interest } from '../../Data/Interests/Interests';
...
const LConversationPage: FunctionComponent = observer(() => {
    ...
    const [timer, setTimer] = useState<number>(
        IS_SIXTY_MIN
            ? global_variables.ConversationTime
            : global_variables.ConversationTime / 2,
    );
    ...
    const init_interest =
        (UserInfoStore.user_info?.interests || [])?.length > 0
            ? UserInfoStore.user_info?.interests?.[0]
            : 'Science';

    // const GPT_PROMPT = new_conversation_prompt({
    //     fullname: UserInfoStore?.user_info?.fullname as string,
    //     topic: InitInterestStore.initial_used
    //         ? get_random_interest()
    //         : (init_interest as string),
    // });
    const GPT_PROMPT: string = AdminStore.admin_data?.conversation_prompt
        ?.replace(
            '/--FULLNAME--/',
        ...
        ?.replace(
            '/--TOPIC--/',
            InitInterestStore.initial_used
                ? get_random_interest()
                : (init_interest as string),
        )
        ...
        onSettled: async data => {
            ...
            if (!data?.error) {
                if (data?.data?.chat_res) {
                    ...
                    if (!InitInterestStore.initial_used) {
    ...
    }, [gpt_req_mutate, GPT_PROMPT, mssgSent, allMessages?.length]);
    ...
    }, [timer, allMessages?.length, navigation, hasSkipped]);
    ...
    useEffect(() => {
        ...
        if (timer === 8 && !hasSkipped) {
            const convey_mssg = 'The Lesson is over for today, Good Job!';
            setHideInput(true);
            setAllMessages(prev => [
                ...prev,
                {
    ...
    // !Init -> Clear Vocabularies based on New Lesson
    useEffect(() => {
        if (!IS_SIXTY_MIN) {
            const lesson_id = route.params?.lesson_id;

            const get_lesson_id = async (): Promise<string | undefined> => {
                try {
                    const data = await SInfo.getItem(
                        SECURE_STORAGE_CLESSON_DATA,
                        {
            ...
            const check_voc_data = async () => {
                const saved_lesson = await get_lesson_id();

                if (saved_lesson) {
                    const saved_l_data = JSON.parse(saved_lesson);

                    if (saved_l_data !== lesson_id) {
                        VocabularyInfoStore.delete_vocabularies();
                        await SInfo.setItem(
                            SECURE_STORAGE_CLESSON_DATA,
    ...
    }, [route.params?.lesson_id, IS_SIXTY_MIN]);
...
export default LConversationPage;
