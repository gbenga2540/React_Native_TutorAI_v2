...
const LGrammarPage: FunctionComponent = observer(() => {
    ...
    // const GPT_PROMPT = new_grammar_prompt({
    //     fullname: UserInfoStore?.user_info?.fullname as string,
    //     topic: route.params?.topic,
    //     subtopic: route.params?.sub_topic,
    // });
    const GPT_PROMPT: string = AdminStore.admin_data?.grammar_prompt
        ?.replace(
            '/--FULLNAME--/',
    ...
    useEffect(() => {
        ...
        if (timer === 8 && !hasSkipped) {
            setHideInput(true);
            const convey_mssg =
                "Thank you for working hard on the Grammar Session, now let's move on to the next session.";
            setAllMessages(prev => [
                ...prev,
                {
                    role: 'assistant',
